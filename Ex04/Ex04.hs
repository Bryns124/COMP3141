...
wf Null = True
wf (Leaf _ _) = True
wf (Node i l r) = balanced (Node i l r) && (i == nodeInfo l <> nodeInfo r) && wf l && wf r
...
pop (Node _ l r)
  | unPriority (maxPrio l) >= unPriority (maxPrio r) = case pop l of
    (l', a) -> (node l' r, a)
...
insert p a (Node _ l r)
    | sizeOf l <= sizeOf r = node (insert p a l) r
    | otherwise = node l (insert p a r)
