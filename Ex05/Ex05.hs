...
extend queens = do
  newQueen <- [0..7]
  guard (isSafe newQueen queens)
  return (queens ++ [newQueen])
  where
    isSafe :: Row -> Queens -> Bool
    isSafe newQueen queens = all (safe newQueen) (zip queens [0..])
    safe :: Row -> (Row, Int) -> Bool
    safe newQueen (queen, columnDiff) = newQueen /= queen && abs (newQueen - queen) /= (length queens - columnDiff)
...
solutionStartWith = error "TODO: Implement solutionStartWith"
