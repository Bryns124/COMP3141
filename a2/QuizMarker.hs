...
parseBool = 
  (keyword "true" >> return True) `orelse` (keyword "false" >> return False)
...
parseDouble = do
  sign <- (keyword "-" >> return negate) `orelse` return id
  preDec <- parseDigits
  postDec <- (keyword "." >> parseDigits) `orelse` return "0"
  let doubleStr = preDec ++ "." ++ postDec
  case readMaybe doubleStr of
    Just double -> return (sign double)
    Nothing -> abort
  where
    parseDigits :: Parser String
    parseDigits = do
      digits <- parsePred isDigit
      if null digits then abort else return digits
...
parseString = do
  ...
  case readMaybe ('"' : str ++ "\"") of
    Just parsedStr -> return parsedStr
    Nothing -> abort
  where
    parseInsideStr = do
      c <- parseChar
      if c == '\"' then return ""
        ...
        nextChar <- parseChar
        leftovers <- parseInsideStr
        return (c : nextChar : leftovers)
      else do
        leftovers <- parseInsideStr
        return (c : leftovers)
...
parseList l r p = do
  ...
  where
    ...
    parseElements =
      (do x <- parseElement
          xs <- parseWhile (keyword "," >> parseElement)
          return (x:xs))
      `orelse` return []
...
parseJSON = do
  keyValues <- parseList '{' '}' parseKeyValue
...
toSubmission (JSON xs) = do
  session <- lookup "session" xs >>= getString
  quizName <- lookup "quiz_name" xs >>= getString
  student <- lookup "student" xs >>= getString
  answersDataList <- lookup "answers" xs >>= getList
  answers <- mapM (\d -> getList d >>= mapM toInt) answersDataList
  timeStr <- lookup "time" xs >>= getString
  time <- toTime timeStr
  ...
  where
    toInt :: Data -> Maybe Int
    toInt (Number n)
      | n > 0 && n == fromInteger (round n) = Just $ round n
    toInt _ = Nothing
...
toSubmissions (JSON xs) = mapM convertJSON xs
  where
    convertJSON (key, val) = do
      valJSON <- getJSON val
      submission <- toSubmission valJSON
...
-- parseQuiz :: Parser Quiz
-- parseQuiz input = do
--   let (deadlineInp:questionsInp) = lines input
--   deadline <- toTime deadlineInp
--   questions <- mapM parseQuestion questionsInp
--   return (Quiz deadline questions)
--   where
--     parseQuestion :: Parser Question
--     parseQuestion = error "Implement parseQuestion"
...
unitTests1 :: Bool
unitTests1 = u1 && u2 && u3 && u4 && u5 && u6 && u7 && u8
  where
